...
def show_financial_aid_page():
    # Load data
    try:
        hf_custom = load_data(normalize_url("chloecodes/IPEDS_SFA", "sfa.csv"))
    ...
    with col4:
        ...
        fig = plot_aid_received_linechart(hf_custom, selected_institution)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col10:
        ...
        st.plotly_chart(fig, use_container_width=True)
    with col11:
        fig = plot_category_aid_disbursed(hf_custom, selected_institution, selected_year)
        st.plotly_chart(fig, use_container_width=True)
    
    plot_total_aid_disbursed(hf_custom, selected_institution, selected_year)